                               ...
                               int namelen)
{
    ...
    net_nsid = nsid;
    net_nsid.time_low = my_htonl(net_nsid.time_low);
    net_nsid.time_mid = my_htons(net_nsid.time_mid);
    net_nsid.time_hi_and_version = my_htons(net_nsid.time_hi_and_version);
...
void format_uuid_v3or5(uuid_t *uuid, unsigned char hash[16], int v)
{
    ...
    uuid->time_low = my_ntohl(uuid->time_low);
    uuid->time_mid = my_ntohs(uuid->time_mid);
    uuid->time_hi_and_version = my_ntohs(uuid->time_hi_and_version);
...
void gen_uuid(char *uuid_str, int size)
{
	uuid_t u;
	uuid_create(&u);
	memset(uuid_str, 0, size);
	sprintf(uuid_str, "%8.8x%4.4x%4.4x%2.2x%2.2x%2.2x%2.2x%2.2x%2.2x%2.2x%2.2x", 
